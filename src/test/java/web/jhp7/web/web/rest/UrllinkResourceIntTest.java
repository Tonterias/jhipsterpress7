...
import web.jhp7.web.service.UrllinkService;
...
import web.jhp7.web.service.UrllinkQueryService;
...
public class UrllinkResourceIntTest {
    ...
    @Test
    @Transactional
    public void getAllUrllinksByPostIsEqualToSomething() throws Exception {
        // Initialize the database
        Post post = PostResourceIntTest.createEntity(em);
        em.persist(post);
        em.flush();
        urllink.addPost(post);
        urllinkRepository.saveAndFlush(urllink);
        Long postId = post.getId();

        // Get all the urllinkList where post equals to postId
        defaultUrllinkShouldBeFound("postId.equals=" + postId);

        // Get all the urllinkList where post equals to postId + 1
        defaultUrllinkShouldNotBeFound("postId.equals=" + (postId + 1));
    }
