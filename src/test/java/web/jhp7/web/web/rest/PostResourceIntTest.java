...
import web.jhp7.web.domain.Tag;
import web.jhp7.web.domain.Topic;
import web.jhp7.web.repository.PostRepository;
import web.jhp7.web.service.PostService;
...
import web.jhp7.web.service.PostQueryService;
...
public class PostResourceIntTest {
    ...
    @Test
    @Transactional
    public void getAllPostsByUrllinkIsEqualToSomething() throws Exception {
        // Initialize the database
        Urllink urllink = UrllinkResourceIntTest.createEntity(em);
        em.persist(urllink);
        em.flush();
        post.setUrllink(urllink);
        postRepository.saveAndFlush(post);
        Long urllinkId = urllink.getId();

        // Get all the postList where urllink equals to urllinkId
        defaultPostShouldBeFound("urllinkId.equals=" + urllinkId);

        // Get all the postList where urllink equals to urllinkId + 1
        defaultPostShouldNotBeFound("urllinkId.equals=" + (urllinkId + 1));
    }
